var lang = "en"

var Sql=require("rampart-sql");
var db=process.scriptPath + `/data/${lang}_wikipedia_search`;
var sql=new Sql.init(db);
rampart.globalize(rampart.utils);

var htmltop_format=sprintf('%w',
`<!DOCTYPE HTML>
    <html><head><meta charset="utf-8">
    <style>
        body {font-family: arial,sans-serif;}
        td {position:relative;}
        #showrm {position:relative;}
        .np { cursor:pointer;}
        .itemwrap{ width: calc( 100%% - 70px); position: relative;display: inline-block;}
        .imgwrap {width:100%%;float: left; display:inline-block;position:relative;padding-top:5px;}
        .tstr {color:#07c;vertical-align:top;padding-top:3px;display:inline-block;position:absolute;right:0px;}
        .abs { margin-right:5px;white-space: normal;}
        .urlsp {color:#006621;max-width:100%%;overflow: hidden;text-overflow: ellipsis;white-space:nowrap;display:inline-block;font-size:.90em;}
        .urla {text-decoration: none;font-size:16px;overflow: hidden;text-overflow: ellipsis;white-space:nowrap;display:inline-block; width: 100%%; }
        .b { font-size: 18px; margin-left:4px; }
        .onsw { background-color: #d00; color: white; float:right;}
        .onsw-on { background-color:#0c5;} 
        #res {font-size:12px;padding:15px 10px 0px 0px;}
        #setbox {position:relative; padding:10px; margin:10px; background-color:#eee; border: 1px dotted gray; top:0px; left:0px;}
        #setbox td {white-space:nowrap;}
        .sall{ cursor: pointer;position: absolute;left: -15px;top: 0px;}
        .ib { display: inline-block; }
        .rm {display:none; top:24px; position:absolute; font-size: 15px;width: 12px;text-align: center;cursor:pointer; font-weight: bold;}
        .res {margin-top: 80px;}
        .resi {min-height:20px;position:relative;clear:both;padding-top: 15px;}
        .hm {display:none;}
        .nw { white-space:nowrap;}
    </style>
    </head><body>
    <div id="lc" style="background-color: white; position: fixed; left:0px; top:0px; min-height: 300px; overflow-x: hidden; padding-right: 20px; padding-left: 20px; box-sizing: border-box; width: 200px;">
     <div style="width:180px;height:128px;margin-bottom:15px"><img src="data:image/jpeg;base64," style="width:145px;height:108px;padding-left:16px;margin-top:10px">
      【ＲａｍｐａｒｔＤＢ】</div>
    </div>
    <div id="main" style="padding-bottom:30px;background-color: white; position: absolute; left:200px; top:0px; min-height: 300px; overflow-x: hidden; padding-right: 20px; padding-left: 30px; box-sizing: border-box; width: 768px;">
      <form id="mf" action="">
        <div style="width:100%%">
          <span style="white-space:nowrap;display:block;width:500px;height:39px;position:fixed;background-color: white;z-index:10;border-bottom: lightGray 1px solid; padding-top:15px;padding-bottom:15px">
            <table style="background-color: white; width:100%%">
              <tr>
                <td style="position:relative">
                  <input autocomplete="off" type="text" id="fq" name="q" value="%H" placeholder="Search" style="box-sizing:border-box;min-width:150px;width:100%%;height:30px;font:normal 18px arial,sans-serif;padding: 1px 3px;border: 2px solid #ccc;">
                  <input type=image id="search" style="height:22px;position: absolute; right: 0px;margin: 4px;" src="data:image/svg+xml;base64,">
                </td>
              </tr>
            </table>
          </span>
        </div>
      </form>
      <div class="res">`
);


function searchvec(req) {
    var q=req.query.q ? req.query.q: "";
    var dojson=false;

    if(req.path.path.match(/\.json$/))
        dojson=true;

    var icount=0;  //estimated total number of results, set below
    var endhtml;   // closing tags, set below

    // add the htmltop text to the server's output buffer.
    // See: https://rampart.dev/docs/rampart-server.html#req-printf
    // it includes escaped '%%' values and the 'value="%H"' format code for the query
    if(!dojson)
        req.printf(htmltop_format, q);

    // if there is a query, search for it and format the results.
    // if not, just send the endhtml.
    if(req.query.q) {
        var lstart=performance.now();
        var x = llama_emb.embedTextToFp16Buf(q);
        var lend = performance.now()

        var maxCandidates = 200;
        var maxShow = 20;

        var fullres={};

        sql.set({
            likeprows:maxCandidates,
            qMinWordLen:2,
            minwordlen:5,
            //alequivs: false,
            //alpostproc:true,
            useDerivations: true
        });


        var fstart=performance.now();
        var fvsearch = faiss_wiki_idx.searchFp16(x.avgVec, /*ndocs = */maxCandidates, /* nprobe = */128);
        var fend = performance.now()

        var ids = [];

        fvsearch.forEach(function(r) {
            ids.push(r.id);
        });

        var istart=performance.now();
        var fres = sql.exec("select vecdist(Vec, ?) score, Idsec, Title, Text from wikivecs where Idsec in (?)", {maxRows:-1}, [x.avgVec, ids]);
        var iend = performance.now()
        // number the original likep order
        fres.rows.forEach(function(r,i){
            r.fpos = i+1;
            // also set the Id and paragraph/section number
            r.Id = Math.floor(r.Idsec/1000);
            r.Secnum = r.Idsec - r.Id*1000;
            fullres[r.Idsec] = r;
        });

        var sstart=performance.now();
        var res = sql.query("select vecdist(Vec, ?, 'dot', 'f16')  score, Idsec, Title, Text from wikivecs where Text likep ?", [x.avgVec,q], {maxRows:maxCandidates});
        /*  this seems to work, where "select vdist(Vec, ?)  score, Id, Title, Text, Secnum from vecs4 where Text likep ? order by 1 desc" is going sloooow
            but idk, its still slower than just sorting in javascript, which doesn't make sense.
            var res = sql.exec("select vdist(Vec, ?v) score, Id, Title, Text from vecs4 where Id in (select Id from vecs4 where Text likep ?q) order by 1 desc",
                {q:q, v:x.avgVec}, {maxRows:maxShow}); 

            Anyway, right now, we want to preserve likep order for comparison and display.
        */
        var send=performance.now();

        if(!res || !res.rows) {
            if(dojson)
                return {json: {results:[]}}
            return {txt: sprintf("%3J\n%3J\n", res, sql)}
        }

        var resrows = res.rows;

        // number the original likep order
        resrows.forEach(function(r,i){
            //req.printf("%d: %s<br>", i, r.Title);
            if( fullres[r.Idsec] )
                fullres[r.Idsec].pos=i+1;
            else
            {
                r.pos = i+1;
                // also set the Id and paragraph/section number
                r.Id = Math.floor(r.Idsec/1000);
                r.Secnum = r.Idsec - r.Id*1000;
                fullres[r.Idsec] = r;
            }
        });

        //resrows = fres.rows.concat(resrows);
        resrows = Object.values(fullres);

        // sort by vector similarity
        resrows.sort( function(a,b){
            return b.score - a.score;
        });

        // resort top maxRerank docs using reranker model
        var maxRerank = 30;
        var rrstart=performance.now();
        var lastrr = 1;
        resrows.forEach(function(r,i){
            if(i<maxRerank && lastrr > 0)
            {
                //printf("query: '%s',\ndoc (%d):'%s: %.330s (%d)',\n", q, i+1, r.Title, r.Text, r.Text.length);
                r.rerank = llama_rr.rerank(q, r.Text);
                //printf("score: %f\n\n", r.rerank);
                //lastrr = r.rerank;
            }
            else
                r.rerank = -1000;
        });
        var rrend=performance.now();

        //sort by Rerank, then by vector similarity score
        resrows.sort( function(a,b){
            if(a.rerank==b.rerank)
                return b.score - a.score;
            return b.rerank - a.rerank;
        });

        // print our timings
        if(!dojson)
            req.printf("<b>timings</b>: llama q->vec: %.3fms, faiss: %.3fms sql_idsel: %.3fms likep: %.3fms, llama rerank: %.3fms<br>%aH<br>", 
                lend-lstart, fend-fstart, iend-istart, send-sstart, rrend-rrstart, 'rgb(128,128,128)', sql.errMsg);
        if(!dojson)
            req.printf(`<h3>Likep -> vector sort -> llama reranked search results</h3>
                    <span class="abs urlsp snip"><b>%aH</b>( %aH, vdist, %aH )</span>
                    <br>%d results<br>`, 'black','legend: ', 'blue', 'rerankScore', 'violet','likep/faiss ord', resrows.length);

        var ntshown=0;
        var wasSeen={};
        var i=0;
        var jsonres=[];

        resrows.some(function(row) {
            i++;

            if(ntshown>=maxShow)
                return true;

            if(dojson && row.rerank<0)
                return true;

            if(dojson)
                jsonres.push({
                    score:row.rerank,
                    text: row.Text,
                    title: row.Title,
                    link: sprintf('https://en.wikipedia.org/wiki?curid=%i', row.Id)
                }); 
            else if(!wasSeen[row.Id]) {
                try{
                    var abs = Sql.abstract(row.Text, 400, 'querybest', '@0 ' + q, '%mbH');
                } catch(e){
                    req.printf("<p>%s</p>", e.message);
                    var abs = row.Text.substr(230);
                }
                var rid = row.id * 1000 + row.Secnum;
                    req.printf('<div class="resi" style="padding-top: 15px;">'+
                                '<span class="imgwrap">'+
                                '<span class="itemwrap">'+
                                    '<span class="abs nw">'+
                                      '<a class="urla tar" target="_blank" href="https://en.wikipedia.org/wiki?curid=%i">%s</a>'+
                                    '</span>'+
                                    '<span class="abs urlsp snip">(%aH, %.4f, %aH, %aH) -> https://en.wikipedia.org/wiki?curid=%i</span>'+
                                    '<span class="abs snip"><br>%s</span>'+
                              '</span></span></div>'
                    ,row.Id, row.Title, 'blue', sprintf('%.4f',row.rerank), row.score, 'violet', 
                    "likep:"+ (row.pos? row.pos:-1), 'violet', 'faiss:'+(row.fpos?row.fpos:-1), row.Id, abs);
                ntshown++;
                wasSeen[row.Id]=true;
            }
        });
    }

    if(dojson)
        return {json: {results:jsonres}};

    endhtml='</div></div></body></html>';
    return({html:endhtml});
}

//export the main search function
module.exports=searchvec;

// -fin-
